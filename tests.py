...
def skip_not_implemented(func):
    ...
    def wrapper():
        try:
            ...
            raise SkipTest("Test {0} is skipped {1}".format(func.__name__, exc))
...
def preprocess_ncc_zeros_test():
    ...
    assert n.shape == (2 * ncc_size - 1, 2 * ncc_size - 1, 3 * ncc_size * ncc_size)
...
def preprocess_ncc_delta_test():
    ...
    correct = np.zeros(
        (2 * ncc_size - 1, 2 * ncc_size - 1, 3 * ncc_size**2), dtype=np.float32
    )
    correct[ncc_half:-ncc_half, ncc_half:-ncc_half, :] = -1.0 / (
        ncc_size * math.sqrt(3 * ncc_size**2 - 3)
    )
    ...
    for i in range(ncc_size):
        for j in range(ncc_size):
            correct[
                -(i + ncc_half + 1),
                -(j + ncc_half + 1),
                ncc_size**2 * 0 + ncc_size * i + j,
            ] = x
            correct[
                -(i + ncc_half + 1),
                -(j + ncc_half + 1),
                ncc_size**2 * 1 + ncc_size * i + j,
            ] = x
            correct[
                -(i + ncc_half + 1),
                -(j + ncc_half + 1),
                ncc_size**2 * 2 + ncc_size * i + j,
            ] = x
...
def correlated_ncc_test():
    ...
    assert (np.abs(ncc[:, :ncc_half]) < 1e-5).all()
    assert (np.abs(ncc[:, -ncc_half:]) < 1e-5).all()
    assert (np.abs(ncc[ncc_half:-ncc_half, ncc_half:-ncc_half] - 1) < 1e-5).all()
...
def zero_ncc_test():
    ...
    image1 = np.zeros((2 * ncc_size - 1, 2 * ncc_size - 1, 3), dtype=np.float32)
    image2 = image1
...
def offset_and_scale_ncc_test():
    ...
    assert (np.abs(ncc[:, :ncc_half]) < 1e-6).all()
    assert (np.abs(ncc[:, -ncc_half:]) < 1e-6).all()
    assert (np.abs(ncc[ncc_half:-ncc_half, ncc_half:-ncc_half] - 1) < 1e-6).all()
...
def compute_photometric_stereo_test():
    ...
    lights = np.array(((0, 1, 0), (0, 0, 1), (1, 0, 0)), dtype=np.float32)
